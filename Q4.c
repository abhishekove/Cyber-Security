...
void sdes_encrypt()
{
    permute(ptcopy,pt,ip,8);

    fkey(k1);
    swap();
    fkey(k2);

    permute(ct,ptcopy,ipinv,8);
}
void sdes_decrypt()
{
    permute(ptcopy,ct,ip,8);

    fkey(k2);
    swap();
    fkey(k1);

    permute(dct,ptcopy,ipinv,8);
}
void keygen()
{
...
void fkey(char keyarr[])
{
    ...
    for(i=0;i<8;i++)
        temp[i]=(char)(48 + ((temp[i]- '0')^(keyarr[i] - '0'))); // Performing XOR On temp with key arr
